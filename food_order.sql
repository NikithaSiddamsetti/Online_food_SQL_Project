...
create table Menu (menu_id int primary key , restaurant_id tinyint, item_name varchar(100), price decimal(10,2), availability varchar(20),Foreign key (restaurant_id) references Restaurants(restaurant_id));
...
#2.2. Update the order status from Preparing to Delivered
...
DELETE FROM Orders WHERE customer_id = 305 AND order_status = 'Cancelled';
...
select lower(item_name) as item_names from menu;
...
select * from customers;
select concat_ws(' ',customer_name,city) as name_city from customers;

#9. Replace the word Chicken with Grilled in product names.
select * from menu;
select item_name,replace(item_name,'Chicken','Grilled') as new_item from menu;
...
select date_format(payment_date, '%d-%M-%Y') as payment_date from payments;
...
select * from restaurants;
select r.restaurant_name from Restaurants r join Menu m on r.restaurant_id = m.restaurant_id where m.price = (select MAX(price) from Menu);

#27 .Display the restaurant that serves the most expensive food item  
select * from menu;
select * from restaurants;
select  item_name from menu  where price = (select MAX(price) from menu);
...
# 36. Create a before delete trigger on orders table whenever order is deleted store the deleted order_details in order archieve.
CREATE TABLE Order_Archive (order_id INT,customer_id INT,menu_id INT,quantity INT,order_date DATE,order_status VARCHAR(30),deleted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
